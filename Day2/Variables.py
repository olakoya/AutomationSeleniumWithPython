...
It can be string, integer, triple, complex, dictionary, and range datatypes.
Python is a dynamic type of programming language.
Every Variable is associated to a specific datatype.
A dynamic programming language based on the variable, the datatype of the variable is automatically accepted.
There are different ways of creating variable with the same values.
There is a result memory location as seen in the example below;
In Java you need to declare a variable and provide the datatype of the variable but not with python. 
Once you declare the variable with a datatype you can store some value.
"""

a = 10 # "a" is a variable and the memory location storing a value of "10" datatype
...
'''
Remember that a variable is a container in a memory location to store data as value.
...
AFter using a variable, it is highly recommended to delete that variable if it's no longer required in order for the corresponding 
...
But we cannot delete the elements present inside the immutable object.
E.g
'''
...
--------------------------------
E.g from the above script "welcome", "w" in welcome is an important element likewise the other leters in "welcome" word.
And all these letters are available in the string datatype "welcome".
However, one cannot delete a letter i.e. an element from the string datatype or immutable object "welcome" e.g "w' or "l" or "m" can't be deleted from the word or object.
We can only access the "welcome" object by using indexing
E.g (below script is the continuation of the above script)
'''
